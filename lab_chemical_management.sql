...
CREATE TABLE user (
    ...
    CONSTRAINT fk_user_lab FOREIGN KEY (lab_id) REFERENCES laboratory(lab_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE chemical (
    ...
    status VARCHAR(20) DEFAULT '正常',
    remark VARCHAR(255),
    CONSTRAINT fk_chemical_category FOREIGN KEY (category_id) REFERENCES chemical_category(category_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE inventory (
    ...
    CONSTRAINT fk_inventory_chemical FOREIGN KEY (chemical_id) REFERENCES chemical(chemical_id),
    CONSTRAINT fk_inventory_lab FOREIGN KEY (lab_id) REFERENCES laboratory(lab_id),
    CONSTRAINT uq_inventory UNIQUE (chemical_id, lab_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE inbound_record (
    ...
    remark VARCHAR(255),
    CONSTRAINT fk_inbound_chemical FOREIGN KEY (chemical_id) REFERENCES chemical(chemical_id),
    CONSTRAINT fk_inbound_lab FOREIGN KEY (lab_id) REFERENCES laboratory(lab_id),
    CONSTRAINT fk_inbound_user FOREIGN KEY (operator_user_id) REFERENCES user(user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE apply_record (
    ...
    CONSTRAINT fk_apply_lab FOREIGN KEY (lab_id) REFERENCES laboratory(lab_id),
    CONSTRAINT fk_apply_chemical FOREIGN KEY (chemical_id) REFERENCES chemical(chemical_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE approval_record (
    ...
    CONSTRAINT fk_approval_apply FOREIGN KEY (apply_id) REFERENCES apply_record(apply_id),
    CONSTRAINT fk_approval_user FOREIGN KEY (approver_user_id) REFERENCES user(user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE usage_return_record (
    ...
    remark VARCHAR(255),
    CONSTRAINT fk_usage_apply FOREIGN KEY (apply_id) REFERENCES apply_record(apply_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE training (
    ...
    remark VARCHAR(255),
    CONSTRAINT fk_training_user FOREIGN KEY (organizer_user_id) REFERENCES user(user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE training_participant (
    ...
    CONSTRAINT fk_tp_user FOREIGN KEY (user_id) REFERENCES user(user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE inspection_record (
    ...
    result VARCHAR(50),
    remark VARCHAR(255),
    CONSTRAINT fk_inspection_lab FOREIGN KEY (lab_id) REFERENCES laboratory(lab_id),
    CONSTRAINT fk_inspection_user FOREIGN KEY (inspector_user_id) REFERENCES user(user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE rectification_record (
    rectify_id INT PRIMARY KEY AUTO_INCREMENT,
    ...
    CONSTRAINT fk_rectify_inspection FOREIGN KEY (inspection_id) REFERENCES inspection_record(inspection_id),
    CONSTRAINT fk_rectify_user FOREIGN KEY (responsible_user_id) REFERENCES user(user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE safety_event (
    ...
    CONSTRAINT fk_event_lab FOREIGN KEY (lab_id) REFERENCES laboratory(lab_id),
    CONSTRAINT fk_event_user FOREIGN KEY (report_user_id) REFERENCES user(user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE event_handle_record (
    ...
    CONSTRAINT fk_handle_event FOREIGN KEY (event_id) REFERENCES safety_event(event_id),
    CONSTRAINT fk_handle_user FOREIGN KEY (handler_user_id) REFERENCES user(user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE attachment (
    ...
    CONSTRAINT fk_attachment_event FOREIGN KEY (event_id) REFERENCES safety_event(event_id),
    CONSTRAINT fk_attachment_training FOREIGN KEY (training_id) REFERENCES training(training_id),
    CONSTRAINT fk_attachment_user FOREIGN KEY (upload_user_id) REFERENCES user(user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE INDEX idx_user_lab_id ON user(lab_id);
...
INSERT INTO user (username, password, real_name, gender, phone, email, role_id, lab_id, status) VALUES
...
SELECT 
    ...
    a.status
FROM apply_record a
JOIN user u ON a.apply_user_id = u.user_id
...
JOIN user u ON e.report_user_id = u.user_id;
...
BEGIN
    ...
    DECLARE v_chemical_id INT;
    DECLARE v_lab_id INT;
    DECLARE v_returned_quantity DECIMAL(10,2);

    SET v_apply_id = NEW.apply_id;
    SET v_returned_quantity = NEW.returned_quantity;
...
CREATE PROCEDURE sp_get_lab_inventory(IN p_lab_id INT)
BEGIN
    SELECT 
        l.lab_name,
        c.chemical_name,
...
CREATE PROCEDURE sp_get_apply_statistics(IN p_start DATETIME, IN p_end DATETIME)
BEGIN
    SELECT 
        c.chemical_name,
        COUNT(a.apply_id) AS apply_count,
        SUM(a.apply_quantity) AS total_apply_quantity
    FROM apply_record a
    JOIN chemical c ON a.chemical_id = c.chemical_id
...
BEGIN
    SELECT
        ...
        SUM(CASE WHEN e.status = '已完成' THEN 1 ELSE 0 END) AS finished_count,
        SUM(CASE WHEN e.status <> '已完成' THEN 1 ELSE 0 END) AS unfinished_count
    FROM safety_event e
    JOIN laboratory l ON e.lab_id = l.lab_id
    GROUP BY l.lab_name
