...
exports.getApplyList = async (req, res) => {
  try {
    ...
    res.json({ message: '获取申请列表成功', data: rows });
  } catch (error) {
...
exports.getPendingApply = async (req, res) => {
  try {
    const [rows] = await pool.query(`SELECT * FROM v_pending_apply`);
    res.json({ message: '获取待审批申请成功', data: rows });
  } catch (error) {
...
exports.createApply = async (req, res) => {
  try {
    ...
    res.json({ message: '提交申请成功', apply_id: result.insertId });
  } catch (error) {
...
exports.approveApply = async (req, res) => {
  try {
    ...
    res.json({ message: '审批成功', approval_id: result.insertId });
  } catch (error) {
